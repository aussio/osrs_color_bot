...
def __load_font(font: str) -> Dict[str, cv2.Mat]:
    ...
    """
    PATH = pathlib.Path().resolve().parent.joinpath("fonts", font)
...
def extract_text(screenshot, font: dict, color: Union[clr.Color, List[clr.Color]], exclude_chars: Union[str, List[str]] = problematic_chars) -> str:
    """
    ...
    image = clr.isolate_colors(screenshot, color)
    result = ""
    char_list = []
    for key in font:
